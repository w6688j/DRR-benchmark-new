...
class RunRnnatt17:
        ...
        for epoch in range(EPOCH):
            ...
            for data in sentences:
                ...
                if ((step + 1) % BATCH_SIZE == 0):
                    sentenceArr = Variable(torch.LongTensor(sentenceArr)).to(device)
                    labelArr = Variable(torch.LongTensor(labelArr)).to(device)
    ...
    def runTest(self):
        ...
        # 加载模型
        RNNAtt17Model = torch.load(self.model_path).to(device)
